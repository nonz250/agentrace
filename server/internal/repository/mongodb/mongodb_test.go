...
// testDB creates a MongoDB database connection for testing.
// Requires MONGODB_URL environment variable to be set.
// Example: MONGODB_URL=mongodb://localhost:27017/agentrace_test
func testDB(t *testing.T) (*DB, func()) {
	t.Helper()

	databaseURL := os.Getenv("MONGODB_URL")
	if databaseURL == "" {
		t.Skip("MONGODB_URL not set, skipping MongoDB integration tests")
	}
	...
	if err := db.createIndexes(ctx); err != nil {
		t.Fatalf("failed to recreate indexes: %v", err)
	}
