...
// Requires TURSO_URL environment variable to be set.
// Example: TURSO_URL=libsql://[db-name]-[org].turso.io?authToken=[token]
func testDB(t *testing.T) (*DB, func()) {
	t.Helper()

	databaseURL := os.Getenv("TURSO_URL")
	if databaseURL == "" {
		t.Skip("TURSO_URL not set, skipping Turso integration tests")
	}
