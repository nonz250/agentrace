...
func NewRouter(cfg *config.Config, repos *repository.Repositories) http.Handler {
	...
	planCommentThreadHandler := NewPlanCommentThreadHandler(repos)
	...
	apiBearerOrSession.HandleFunc("/sessions/{id}", sessionHandler.Update).Methods("PATCH")
	apiBearerOrSession.HandleFunc("/sessions/{id}", sessionHandler.Delete).Methods("DELETE")
	...
	apiBearerOrSession.HandleFunc("/plans/{id}/comments", planCommentThreadHandler.Create).Methods("POST")
	apiBearerOrSession.HandleFunc("/plans/{id}/comments/{threadId}", planCommentThreadHandler.Delete).Methods("DELETE")
	apiBearerOrSession.HandleFunc("/plans/{id}/comments/{threadId}/resolve", planCommentThreadHandler.Resolve).Methods("POST")
	apiBearerOrSession.HandleFunc("/plans/{id}/comments/{threadId}/messages", planCommentThreadHandler.AddMessage).Methods("POST")
	apiBearerOrSession.HandleFunc("/plans/{id}/comments/{threadId}/messages/{messageId}", planCommentThreadHandler.UpdateMessage).Methods("PATCH")
	apiBearerOrSession.HandleFunc("/plans/{id}/comments/{threadId}/messages/{messageId}", planCommentThreadHandler.DeleteMessage).Methods("DELETE")
	...
	apiOptional.HandleFunc("/plans/{id}/comments", planCommentThreadHandler.List).Methods("GET")
